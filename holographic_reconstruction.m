...
% Constants:
res = (5.97e-6); % res = (3.85e-6); % [m] Pixel size of Nikon 3200D; 4.3e-6 for Canon Rebel T2i
...
date_def = SN_def(1:6); % date_def = [year_def,'_', month_def,'_', day_def];    
date_base = SN_base(1:6); % date_base = [year_base,'_',month_base,'_',day_base];
date_plot = [year_def, month_def, day_def];

shot_def = SN_def(7:end);
shot_base = SN_base(7:end);

hol_def_rgb = imread(['/Users/apple/Documents/ZaPHD/DHI',date_def,'/DHI',date_def,shot_def,'.jpg']);
% hol_def_rgb = imread(['H:\Raw Holograms\',date_def,'\',date_def, '_', shot_def,'.jpg']);
hol_base_rgb = imread(['/Users/apple/Documents/ZaPHD/DHI',date_base,'/DHI',date_base,shot_base,'.jpg']);
% hol_base_rgb = imread(['H:\Raw Holograms\',date_base,'\',date_base, '_', shot_base,'.jpg']);
...
x_twin = x(x_min_ind:x_max_ind); px_icr = 2; % px_icr = pixel increment of toss away
x_twin = x_twin(1:px_icr:end); % x_twin = x_twin(1:10:end); % toss away some pixels to save computation time on abel invert
y_twin = y(y_min_ind:y_max_ind);
y_twin = y_twin(1:px_icr:end); % y_twin = y_twin(1:10:end);

twin_img = phase_diff(y_min_ind:y_max_ind,x_min_ind:x_max_ind);
twin_img = twin_img(1:px_icr:end,1:px_icr:end); % twin_img = twin_img(1:10:end,1:10:end);
